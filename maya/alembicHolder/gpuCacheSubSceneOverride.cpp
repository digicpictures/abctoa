...
// gpuCache node-level callbacks are registered in AlembicHolder::SubSceneOverride.
...
void NodeDirtyCallback(MObject& node, MPlug& plug, void* clientData)
{
    ...
    if (attr.name() == "cacheSelectionPath") {
        static_cast<SubSceneOverride*>(clientData)->dirtyVisibility();
    } else if (ModelCallbacks::getInstance().affectAppearance(attr.name())) {
        static_cast<SubSceneOverride*>(clientData)->dirtyEverything();
    }
}
...
// AlembicHolder::SubSceneOverride.
...
public:
                           ...
                           const Drawable&     drawable)
    {
        ...
        if (fBoundingBoxItem) {
            const auto& boundingBox = drawable.getBounds();
                           ...
                           const size_t        nbIndexGroups)
    {
        ...
        if (fShadedItems.empty()) {
            ...
            for (size_t groupId = 0; groupId < nbIndexGroups; groupId++) {
                ...
                renderItem->setDrawMode((MGeometry::DrawMode)(MGeometry::kShaded));
...
public:
    ...
    virtual void visit(const IXformDrw&   xform)
    {
        ...
        //const auto& xf = xform.getXform();
        //const auto& p = xf.getParent();
        //bool isTop = xform.getXform().getParent() && subNode.getName() == "|";
        //if (!isTop) {
        //    fLongName += "|";
    ...
    void updateRenderItems(const IPolyMeshDrw& shape)
    {
...
public:
    ...
    virtual void visit(const AlembicHolder::IPolyMeshDrw&   shape)
    {
        // Update the sub-node.
        assert(fShapeSubNodeIndex < fSubNodeItems.size());
        if (fShapeSubNodeIndex < fSubNodeItems.size()) {
            static_cast<DERIVED*>(this)->update(shape, fSubNodeItems[fShapeSubNodeIndex]);
        }
        fSubNodeIndex++;
        fShapeSubNodeIndex++;
    }
...
public:
    ...
    void update(const IPolyMeshDrw&      shape,
                SubNodeRenderItems::Ptr& subNodeItems)
    {
        ...
        bool visibility = fVisibility && sample->visibility() &&
                          fSubSceneOverride.isPartVisible(shape.getIObject().getFullName());
    ...
    virtual void visit(const IXformDrw&   xform)
    {
        ...
        fVisibility = fVisibility && xform.isVisible();

        ParentClass::visit(xform);
    }
...
public:
    ...
    virtual void visit(const IXformDrw&   xform)
    {
        ...
        fMatrix = xform.getMMatrix() * fMatrix;
...
bool SubSceneOverride::isPartVisible(const std::string& partName) const
{
    assert(fShapeNode);
    if (!fShapeNode)
        return false;
    ...
    const auto& selectionKey = cache->m_currselectionkey;
    if (selectionKey.empty())
        return true; // No selection key: everything is visible.

    return pathInJsonString(partName, selectionKey);
}
